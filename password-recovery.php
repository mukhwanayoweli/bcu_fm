...
if(isset($_POST['send'])){
    ...
    if($row2 > 0) {
        ...
        try {
            ...
            $mail->Username = 'yowelimukhwana@gmail.com';    // SMTP username
            $mail->Password = '$2y$10$GmSoomDPYcAKsx5lPtNhfucqpfnkgq4ohP/hHS.Mveo8FdzBVuQna';           // SMTP password
            $mail->SMTPSecure = 'tls';                  // Enable TLS encryption, `ssl` also accepted
            $mail->Port = 587;                          // TCP port to connect to

            // Sender and recipient settings
            $mail->setFrom('yowelimukhwana@gmail.com', 'my new website');
            $mail->addAddress($toemail);   // Add a recipient
            $mail->isHTML(true);  // Set email format to HTML

            // Email content
            $mail->Subject = $subject;
            $mail->Body = 'Dear '.$fname.'<p>'.$message.'</p>';
...
In the modified code, make sure to replace  `'your-email@gmail.com'`  with your actual Gmail address and  `'your-password'`  with your Gmail password. Also, ensure that you have the necessary PHPMailer files in the  `vendor`  directory and that the  `config.php`  file is correctly included.